...
const getStates = async (district, currentDate, prevState) => {
  try {
    axios({
      ...
      data18.length > 0 ? prevState[0] !== data18.length ?prevState[0]= changeState(prevState[0], data18) : console.log('same data') :console.log("No 18+ record Found")
      data45.length > 0 ? prevState[1] !== data45.length ?prevState[1] = changeState(prevState[1],data45) : console.log('same data') :console.log("No 45+ record Found")
    });
  } catch (err) {
...
const changeState = (state, data) => {
  var val = state
  console.log("mailing to "+data[0].district_name+ data[0].min_age_limit + `changed from ${val} to ${data.length}`)
  val = data.length;
  mailing(data)
  return val
}
